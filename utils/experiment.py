...
# def adjust_learning_rate_onecycle(optimizer, cur_epoch, base_lr, total_epochs):


def adjust_learning_rate_poly(optimizer, cur_epoch, base_lr, total_epochs):

    lr = base_lr * ((1 - float(cur_epoch) / total_epochs) ** 0.9)
    print("setting learning rate to {}".format(lr))
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr
